...
레퍼런스 선언 시 자료형 앞에 const를 붙여 사용 (함수의 인자 전달 시 인자의 값을 변경할 수 없도록 함)
...
````
struct 구조체이름 {						struct MEMINFO {
	자료형 멤버이름;						 char name[20];		
	...									  int salary; 
} 변수명 또는 객체명;					  };
````

- 구조체이름은 생략 가능하나 대신 변수 또는 객체명을 지정해야함
- 멤버의 선언은 자료형과 멤버; 으로 구성
- 구조체 선언과 함께 변수나 객체의 선언 가능

구조체를 선언하는 방법은 일반 변수와 동일

- C 언어에서는 struct 키워드와 함께 구조체 자료형의 이름 명시
- **C++에서는 struct 키워드 생략 가능**

변수의 선언 =  메모리의 할당

- 구조체 선언 순서대로 멤버의 메모리가 할당 받음

구조체 변수의 선언과 동시에 초기화 시 {} 이용 (배열과 동일)

- 구조체의 크기는 각 멤버들의 크기를 모두 더한 것과 같거나 클 수도 있음
  (컴파일러의 **메모리 정렬 방식**에 의해 멤버들 사이의 사용되지 않는 바이트가 들어갈 수도 있음)
  **메모리 정렬** : CPU에서 효과적인 메모리 접근을 위해 데이터가 특정한 단위로 정렬되어 그 크기 만큼 접근
- sizeof 연산자를 이용하여 구조체의 크기를 구할 수 있음

구조체 멤버들을 따로 다룰 수 있음

- 구조체 변수명 옆에 . 을 붙이고 멤버의 이름을 붙여 사용
